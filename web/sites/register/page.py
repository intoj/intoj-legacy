...
def Register(req):
	...
	if re.match(r'[A-Za-z0-9_\-]+',username) == None or re.match(r'[A-Za-z0-9_\-]+',username).span()[1] != len(username):
	...
	cur.execute("INSERT INTO users(`username`,`password_sha256`,`password_sha1`,`email`) VALUES('%s','%s','%s','%s');"%(username,password_sha256,password_sha1,email))
